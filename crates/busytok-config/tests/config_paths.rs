...
fn socket_path_is_busytok() {
    ...
    let endpoint = paths
        .control_endpoint()
        .expect("control_endpoint should resolve");
...
fn control_endpoint_is_a_named_pipe_on_windows_and_socket_path_on_unix() {
    ...
    let endpoint = paths
        ...
        .expect("control_endpoint should resolve");
    assert!(
        endpoint.contains("busytok"),
        "endpoint should contain 'busytok': {endpoint}"
    );
    assert!(
        !endpoint.contains("autoken"),
        "endpoint must not contain 'autoken': {endpoint}"
    );
