...
use super::command_runner::{
    launchctl_bootout, launchctl_bootout_strict, launchctl_bootstrap_strict,
    launchctl_domain, launchctl_kickstart_strict, launchctl_label, launchctl_print, current_uid,
    CommandRunner, CommandStatus,
};
use super::launchd_job_snapshot::LaunchdJobSnapshot;
use super::smappservice_bridge::{
    MainThreadExecutor, SMAppServiceHandle, SMServiceStatus,
};
use super::{
    ...
    let entries = match fs::read_dir(launch_agents_dir) {
        ...
        Err(e) => {
            return Err(e).with_context(|| {
                format!("reading {}", launch_agents_dir.display())
            });
        }
    };
...
impl SmAppServiceLifecycle {
    ...
    fn repair_via_ladder(&self, reason: RepairReason) -> Result<EnsureRunningOutcome> {
        ...
        let event_code = match reason {
            RepairReason::StaleBundle => "service_lifecycle.smappservice.repaired_after_stale_bundle",
            RepairReason::VersionSkew => "service_lifecycle.smappservice.repaired_after_version_skew",
        };
        info!(
    ...
    fn probe_service_build_identity(&self) -> Result<Option<String>> {
        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                ...
                let request =
                    ControlRequest::new("service.status", serde_json::json!({}));
                let response = client
                    .call(request)
                    .await
                ...
                match response {
                    ControlResponse::Ok(value) => {
                        let dto: ServiceStatusDto = serde_json::from_value(value)
                            .map_err(|e| anyhow::anyhow!("decoding service.status response: {e}"))?;
                        Ok(Some(dto.version))
                    }
                    ControlResponse::Err(err) => {
                        anyhow::bail!(
                            "service.status RPC error [{}]: {}",
                            err.code,
                            err.message
                        )
                    }
...
pub fn hydration_state(identity: ServiceBuildIdentity) -> crate::desktop_service_status::ServiceBootstrapState {
...
mod tests {
    use super::*;
    use crate::service_lifecycle::command_runner::{CommandRunner, CommandStatus, SystemCommandRunner};
    use std::sync::Arc;
    ...
    impl FakeMacLifecycle {
        ...
        fn version_probe(&self) -> Result<Option<String>> {
            Ok(match self.scenario.version {
                FakeVersion::Matching => Some(ServiceBuildIdentity::current_gui().as_str().to_string()),
                FakeVersion::Mismatch => Some(ServiceBuildIdentity::mismatch().as_str().to_string()),
        ...
        fn do_register(&self) {
            ...
            if matches!(self.scenario.register_effect, FakeRegisterEffect::AutoStarts) {
    ...
    fn cleanup_legacy_launch_agents_removes_handwritten_user_plists() {
        ...
        let result =
            cleanup_legacy_launch_agents(&launch_agents, &runner)
                .unwrap();
        assert!(result.removed_files.contains(&"com.busytok.service.plist".to_string()));
    ...
    fn cleanup_legacy_uses_uid_in_launchctl_label() {
        ...
        let all_args: Vec<String> = runner.calls().iter().flat_map(|c| c.iter().cloned()).collect();
    ...
    fn prod_ensure_running_never_dispatches_to_smappservice_executor() {
        ...
        let runner: Box<dyn CommandRunner> =
            Box::new(LaunchctlPrintRunner::loaded_job_for(&layout.service_binary_path()));
    ...
    fn prod_lifecycle_cleanup_legacy_launch_agents_works_with_real_struct() {
        ...
        let lc = SmAppServiceLifecycle::new_with_executor(
            layout, paths.clone(), platform, executor, probe, runner,
        );
    ...
    fn preflight_rejects_missing_service_binary() {
        ...
        std::fs::write(
            plist_dir.join("com.busytok.service.plist"),
            "<fake/>\n",
        )
        .unwrap();
