...
pub fn resolve_range(
    ...
    let today_date = Date::from_calendar_date(year, Month::try_from(month).expect("valid month"), day)
        .expect("valid date");

    match preset {
        RangePresetDto::Day => {
            let start_ms = rtz.civil_date_to_utc_start_ms(&today_str).expect("valid start");
            let next_date = rtz.next_civil_date(&today_str).expect("valid next date");
            let end_ms = rtz.civil_date_to_utc_start_ms(&next_date).expect("valid end");
            ResolvedRange {
                start_ms,
                end_ms,
        ...
        RangePresetDto::Week => {
            ...
            let start_ms = rtz.civil_date_to_utc_start_ms(&start_date_str).expect("valid start");
            let end_ms = rtz.civil_date_to_utc_start_ms(&end_date_str).expect("valid end");
...
pub fn heatmap_days(rtz: &ReportingTimezone) -> Vec<HeatmapDayWindow> {
    ...
    let start_date = Date::from_calendar_date(start_year, month_enum, today_day)
        .unwrap_or_else(|_| {
            let next_month = if month_enum == Month::December {
                Month::January
            } else {
    ...
    let today_date = Date::from_calendar_date(today_year, month_enum, today_day)
        .expect("valid today date");
...
fn trend_buckets_daily_month(rtz: &ReportingTimezone) -> Vec<TrendBucketWindow> {
    ...
    let month_start =
        Date::from_calendar_date(year, month_enum, 1).expect("valid month start");
    let next_month_date = if month_enum == Month::December {
        ...
        Date::from_calendar_date(year, month_enum.next(), 1)
            .expect("valid next month start")
    };
...
mod tests {
    ...
    fn trend_buckets_year_has_stable_keys() {
        ...
        let buckets = trend_buckets(&rtz, RangePresetDto::Year, WeekdayIndexDto::SUNDAY);
        assert_eq!(buckets.len(), 12);
        let expected_keys: Vec<String> = (1..=12)
    ...
    fn resolve_day_range_is_midnight_to_midnight() {
        let rtz = ReportingTimezone::parse("UTC").unwrap();
        let range = resolve_range(&rtz, 2026, 5, 20, RangePresetDto::Day, WeekdayIndexDto::SUNDAY);
    ...
    fn resolve_week_range_starts_on_monday() {
        ...
        let range = resolve_range(&rtz, 2026, 5, 20, RangePresetDto::Week, WeekdayIndexDto::MONDAY);
    ...
    fn resolve_month_range() {
        let rtz = ReportingTimezone::parse("UTC").unwrap();
        let range = resolve_range(&rtz, 2026, 5, 20, RangePresetDto::Month, WeekdayIndexDto::SUNDAY);
    ...
    fn resolve_year_range() {
        let rtz = ReportingTimezone::parse("UTC").unwrap();
        let range = resolve_range(&rtz, 2026, 5, 20, RangePresetDto::Year, WeekdayIndexDto::SUNDAY);
