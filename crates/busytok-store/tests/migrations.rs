...
fn baseline_plus_cache_metrics_migrations() {
    ...
    // v6: queue_reason column exists on subagent_tasks.
    let subagent_cols: Vec<String> = conn
        ...
        .collect();
    assert!(
        subagent_cols.contains(&"queue_reason".to_string()),
        "missing v6 column queue_reason on subagent_tasks, got: {subagent_cols:?}"
    );
}
...
fn migrations_registered_in_order() {
    assert_eq!(
        ...
        6,
        "expected baseline + cache-metrics + subagent + subagent-task-fields + provider-catalog-and-subagent-route-binding + subagent-queue-reason migrations"
    );
