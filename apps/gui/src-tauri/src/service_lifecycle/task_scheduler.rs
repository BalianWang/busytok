...
const TASK_TEMPLATE: &str = include_str!("../../../../../packaging/windows/assets/task-template.xml");
...
impl TaskSchedulerLifecycle {
    pub fn new() -> Self {
        Self { paths: BusytokPaths::new(), runner: Box::new(SystemCommandRunner) }
    }
...
impl Default for TaskSchedulerLifecycle { fn default() -> Self { Self::new() } }
...
impl ServiceLifecycle for TaskSchedulerLifecycle {
    fn ensure_registered(&self) -> Result<InstallOutcome> {
        ...
        let create_status = self.runner.run("schtasks", &[
            "/Create".into(), "/TN".into(), PlatformPaths::new().service_identifier().to_string(),
            "/XML".into(), path.display().to_string(), "/F".into(),
        ])?;
        ...
        tracing::info!(event_code = "service_lifecycle.task_scheduler.installed", ?outcome);
        Ok(outcome)
    }
    ...
    fn ensure_running(&self) -> Result<EnsureRunningOutcome> {
        ...
        let run_status = self.runner.run("schtasks", &[
            "/Run".into(),
            "/TN".into(),
    ...
    fn status(&self) -> Result<LifecycleStatus> {
        let csv = self.runner.run("schtasks", &[
            "/Query".into(), "/TN".into(), PlatformPaths::new().service_identifier().to_string(),
            "/FO".into(), "CSV".into(), "/V".into(),
        ]);
        match csv {
            Ok(status) => {
                if !status.success {
                ...
                if stdout.contains("\"Running\"") { Ok(LifecycleStatus::Running) }
                else if stdout.contains("\"Disabled\"") { Ok(LifecycleStatus::Disabled) }
                else if stdout.contains("\"Ready\"") { Ok(LifecycleStatus::RegisteredInactive) }
                else {
                    tracing::warn!(
    ...
    fn stop_for_current_session(&self) -> Result<()> {
        ...
        let stop_status = self.runner.run("schtasks", &[
            "/End".into(),
            "/TN".into(),
            PlatformPaths::new().service_identifier().to_string(),
        ]);
        match stop_status {
            Ok(s) if s.success => {
                tracing::info!(
                    event_code = "service_lifecycle.task_scheduler.stopped_for_session"
                );
            }
            Ok(s) => {
                tracing::warn!(
    ...
    fn uninstall(&self) -> Result<()> {
        let _ = self.runner.run("schtasks", &["/Delete".into(), "/TN".into(), PlatformPaths::new().service_identifier().to_string(), "/F".into()]);
...
mod tests {
    use super::*;
    use super::super::command_runner::{FakeCommandRunner, CommandStatus};
    ...
    fn ok_status() -> CommandStatus { CommandStatus { success: true, exit_code: None, stdout: String::new(), stderr: String::new() } }
    ...
    fn status_not_registered() {
        ...
        let paths = BusytokPaths::for_test(tmp.path());
        let runner = FakeCommandRunner::new();
        runner.enqueue("schtasks", "/Query", CommandStatus { success: false, exit_code: None, stdout: String::new(), stderr: "ERROR: The system cannot find the file specified.".into() });
        let lc = TaskSchedulerLifecycle::with_runner(paths, Box::new(runner));
