...
ACTIONS / [Restart service]=secondary  [Diagnostics]=ghost tertiary
```
...
- item：高 `36→32px`，padding `0 12px`，r6，图标 16px / stroke 1.75。
  - **rest**：文字+图标 `--color-text-muted`，透明底。
...
- **overlay 与 page 共享同一 row 语法**（selected/hover/accessory），仅密度与 actions 组织不同。
...
- 能自动化的编码成测试（扩 `tokens.test.ts` / CSS lint）：
...
| `AppShell/Sidebar/OverviewPage/PromptPaletteOverlay` + overview panels | 组件实现 | ↔ 各组件契约段 |
