...
impl LifecycleLogRecord {
    ...
    pub fn approval_needed(
        ...
        let sm_str = match sm_status {
            ...
            crate::service_lifecycle::smappservice_bridge::SMServiceStatus::NotFound => {
                "not_found"
            }
        };
        Self {
...
impl LifecycleCoordinator {
    ...
    pub async fn ensure_running(
        ...
        let from_phase: LifecyclePhase;
        {
            ...
            if state.phase == LifecyclePhase::SuppressedForSession
                && !cause.clears_suppression()
            {
                self.emit_log_internal(LifecycleLogRecord {
                ...
                && cause.clears_suppression()
            {
                ...
                state.last_transition =
                    Some((from, LifecyclePhase::Active));
                self.emit_log_internal(LifecycleLogRecord {
    ...
    pub async fn stop_for_current_session(
        &self,
        cause: LifecycleCause,
    ) -> Result<()> {
        let from_phase: LifecyclePhase;
    ...
    pub async fn suppress_for_session(&self, cause: LifecycleCause) {
        ...
        state.cause = Some(cause);
        state.last_transition =
            Some((from, LifecyclePhase::SuppressedForSession));
        self.emit_log_internal(LifecycleLogRecord {
            event_code: "lifecycle.suppressed_for_session".into(),
            cause: Some(cause.as_str().into()),
            from_state: from.as_str().into(),
            to_state: LifecyclePhase::SuppressedForSession
                .as_str()
                .into(),
            result: LifecycleResult::Ok.as_str().into(),
            launchctl_exit_code: None,
...
mod tests {
    use super::*;
    use crate::desktop_lifecycle_settings::{DesktopLifecycleSettings, DesktopLifecycleSettingsStore};
    use crate::desktop_login_start::DesktopLoginStart;
    use crate::service_lifecycle::{EnsureRunningOutcome, InstallOutcome, LifecycleStatus, ServiceLifecycle};
    ...
    impl ServiceLifecycle for FakeServiceLifecycle {
        fn ensure_registered(&self) -> Result<InstallOutcome> {
            self.actions.lock().unwrap().push("ensure_registered".into());
        ...
        fn stop_for_current_session(&self) -> Result<()> {
            self.actions.lock().unwrap().push("stop_for_current_session".into());
    ...
    impl DesktopLoginStart for FakeLoginStart {
        fn enable_for_future_logins(&self) -> Result<()> {
            self.actions.lock().unwrap().push("enable_for_future_logins".into());
            Ok(())
        }
        ...
        fn enable_for_current_session(&self) -> Result<()> {
            self.actions.lock().unwrap().push("enable_for_current_session".into());
            Ok(())
        }

        fn adopt_current_session(&self) -> Result<()> {
            self.actions.lock().unwrap().push("adopt_current_session".into());
            Ok(())
        }
    ...
    impl RecordingLoginStart {
        fn new() -> Self {
            Self { actions: StdMutex::new(Vec::new()) }
        }
    ...
    impl ServiceLifecycle for CountingLifecycle {
        ...
        fn ensure_running(&self) -> Result<EnsureRunningOutcome> {
            ...
            let response = guard.take().unwrap_or(Ok(EnsureRunningOutcome::AlreadyRunning));
    ...
    async fn quit_exits_app_even_when_service_stop_fails() {
        ...
        assert!(result.is_ok(), "helper must return Ok regardless of stop failures");
        ...
        assert_eq!(exit_calls.load(Ordering::SeqCst), 1, "exit_app must be called once");
    ...
    async fn lifecycle_logging_emits_stable_event_codes() {
        ...
        assert!(!quit_logs.is_empty(), "should emit lifecycle.quit_requested");
    ...
    async fn ensure_running_is_noop_when_session_suppressed_and_not_explicit_launch() {
        ...
        coordinator
            .suppress_for_session(LifecycleCause::Quit)
            .await;
        assert!(coordinator.is_suppressed().await);
    ...
    async fn quit_racing_ensure_prefers_user_intent() {
        ...
        let abort_logs =
            logs_matching(&recorder, "lifecycle.ensure_aborted_quit_in_flight");
        assert!(
            !abort_logs.is_empty(),
    ...
    async fn repair_racing_disable_login_start_is_serialized() {
        ...
        let ensure_logs =
            logs_matching(&recorder, "lifecycle.ensure_already_running");
        let stop_logs =
            logs_matching(&recorder, "lifecycle.stop_for_session");
        assert!(
            !ensure_logs.is_empty(),
            "ensure should emit a log record"
        );
        assert!(
            !stop_logs.is_empty(),
            "stop should emit a log record"
        );
    }
    ...
    async fn concurrent_ensure_calls_coalesce_into_one_transition() {
        ...
        let h1 = tokio::spawn(async move {
            c1.ensure_running(LifecycleCause::Startup).await
        });
        let h2 = tokio::spawn(async move {
            c2.ensure_running(LifecycleCause::Repair).await
        });
        let h3 = tokio::spawn(async move {
            c3.ensure_running(LifecycleCause::WakeFromSleep).await
        });
        ...
        let coalesced_logs =
            logs_matching(&recorder, "lifecycle.ensure_coalesced");
        assert!(
            !coalesced_logs.is_empty(),
            "concurrent ensures should be coalesced; got logs: {:?}",
            recorder.lock().unwrap().iter().map(|r| &r.event_code).collect::<Vec<_>>()
        );
    }
    ...
    async fn repair_failure_logs_launchctl_context() {
        ...
        let result = coordinator
            .ensure_running(LifecycleCause::Repair)
            .await;
    ...
    async fn gui_owned_recovery_uses_service_lifecycle_without_cli_bootstrap_semantics() {
        ...
        let login_start = Arc::new(RecordingLoginStart::new());
        let (coordinator, _recorder) =
            LifecycleCoordinator::for_test(lifecycle_clone, login_start);

        // Multiple operations use the same lifecycle.
        let _ = coordinator.ensure_running(LifecycleCause::Startup).await;
        let _ = coordinator.ensure_running(LifecycleCause::Repair).await;
        let _ = coordinator.stop_for_current_session(LifecycleCause::Quit).await;
    ...
    async fn manual_reopen_persists_cleared_suppression() {
        ...
        let lifecycle: Arc<dyn ServiceLifecycle> =
            Arc::new(FakeServiceLifecycle::new());
        let login_start: Arc<dyn DesktopLoginStart> =
            Arc::new(FakeLoginStart::new());
        let (coordinator, _recorder) =
            LifecycleCoordinator::for_test(lifecycle, login_start);
