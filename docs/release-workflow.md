...
- [ ] 3. 提 PR（分支上 commit + tag）→ 等 review 合入
- [ ] 4. 本地 `git pull main`，`git tag -f` 移到 merge commit，`--force` push tag
...
## 4. 把 tag 移到 merge commit（关键步骤）

PR merge 后，GitHub 产生新的 merge commit。tag 目前还指在 PR 分支的 commit 上，必须移到 merge commit：

```bash
git checkout main
git pull origin main
git tag -f v0.0.11                  # 移动 tag
git push origin v0.0.11 --force     # 推送覆盖
```
...
> **为什么不能直接在 main 上 commit + tag？** main 受保护，无法直接 push。只能从分支提 PR → merge。而 merge 产生的是新 commit，原来的 tag 不在上面。
...
publish-macos:
  ...
  8. create-dmg → 签名 DMG
  9. Apple 公证 + staple
  10. 创建 draft GitHub Release + 上传产物
```
