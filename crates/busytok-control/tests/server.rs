...
async fn normal_rpc_disconnect_does_not_emit_subscription_diagnostics() {
    ...
    let (server, socket_path): (ControlServer, _) = ControlServer::spawn_for_test(Arc::clone(&runtime) as Arc<dyn RuntimeControl>)
        .await
        .unwrap();
...
async fn normal_rpc_disconnect_is_graceful_for_single_connection_handler() {
    let runtime = Arc::new(TestRuntimeControl::with_claude_fixture().await.unwrap());
    let (server, socket_path): (ControlServer, _) = ControlServer::spawn_for_test(runtime).await.unwrap();
    let server = Arc::new(server);
    let accept_task = {
        let server = Arc::clone(&server);
...
async fn method_dispatch_error_preserves_code_and_payload() {
    ...
    let mut client: ControlClient = ControlClient::connect(&socket_path)
        .await
        .unwrap();
    let response = client
