...
impl BusytokSupervisor {
    ...
    fn run_detached_scan_for_sources(
        ...
        let gen_id = match self.generation_manager.active_generation_id() {
            ...
            None => {
                ...
                self.generation_manager.activate_generation(new_id.clone())?;
                new_id
            }
        };
    ...
    fn current_scan_state(&self) -> Result<&'static str> {
        let db = self.read_query_database()?;
        Self::scan_state_from_conn(db.conn(), busytok_config::service_marker::exists(self.paths.data_dir()))
    }
...
impl RuntimeControl for BusytokSupervisor {
    ...
    async fn overview_trend(
        ...
        let trend_dto_buckets: Vec<OverviewTrendBucketDto> = if is_iana_day {
            ...
            let first_date = buckets
                .first()
                .map(|b| rtz.local_date_for_timestamp_ms(b.start_ms).unwrap_or_default())
            ...
            let daily_rows: Vec<busytok_store::read_models::DailyUsageTrendRow> = self
                .run_read_with_mode("overview.trend", "overview_trend_daily", true, move |conn| {
        ...
        let has_cost = trend_dto_buckets.iter().any(|b| {
            b.cost_usd.as_ref().is_some_and(|c| *c > 0.0)
        });
        let has_no_cost = trend_dto_buckets.iter().any(|b| b.tokens > 0 && b.cost_usd.is_none());
    ...
    async fn overview_heatmap(
        ...
        let heatmap = if use_fast_path {
            ...
            let daily_rows: Vec<busytok_store::read_models::DailyUsageTrendRow> = self
                .run_read_with_mode("overview.heatmap", "overview_heatmap_daily", true, move |conn| {
                    busytok_store::read_queries::read_overview_trend_from_daily_usage(
                        conn,
                        &tz_name,
                        &first_date,
