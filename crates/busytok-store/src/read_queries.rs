...
pub fn read_overview_trend_from_daily_usage(
    ...
    let rows = stmt
        .query_map(params![timezone, start_date, end_date, generation_id], |row| {
...
mod tests {
    ...
    fn read_overview_trend_from_daily_usage_returns_aggregated_rows() {
        ...
        let rows = read_overview_trend_from_daily_usage(conn, "Asia/Shanghai", "2026-06-10", "2026-06-11", "gen-test").unwrap();
    ...
    fn read_overview_summary_from_daily_usage_aggregates_across_dates() {
        ...
        let summary = read_overview_summary_from_daily_usage(conn, "Asia/Shanghai", "2026-06-10", "2026-06-11", "gen-test").unwrap();
    ...
    fn daily_usage_generation_isolation() {
        use crate::write_queries::{insert_usage_events_batch_returning_inserted, upsert_daily_usage_for_events};
        ...
        let summary_a = read_overview_summary_from_daily_usage(
            conn, rtz.canonical_name(), &date, &date, "Gen-A",
        ).unwrap();
        ...
        let summary_b = read_overview_summary_from_daily_usage(
            conn, rtz.canonical_name(), &date, &date, "Gen-B",
        ).unwrap();
    ...
    fn overview_fast_path_and_iana_path_agree_on_same_fixture_utc() {
        ...
        use crate::write_queries::{
            ...
            update_materialized_aggregates_from_events,
            upsert_daily_usage_for_events,
        };
        use busytok_domain::ReportingTimezone;
        ...
        let mut evt1 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-contract-1", busytok_domain::AgentKind::ClaudeCode,
        );
        ...
        let mut evt2 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-contract-2", busytok_domain::AgentKind::Codex,
        );
        ...
        let iana =
            read_overview_summary_from_daily_usage(conn, rtz.canonical_name(), &date, &date, "gen-contract-test")
                .unwrap();
        ...
        let mut evt3 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-contract-3", busytok_domain::AgentKind::ClaudeCode,
        );
        evt3.timestamp_ms = ts1 + 3600_000; // same UTC day, different gen
        evt3.total_tokens = 50;
        evt3.cost_usd = Some(0.005);

        let inserted_gen2 = insert_usage_events_batch_returning_inserted(
            conn,
            &[evt3],
            "gen-other",
        )
    ...
    fn overview_trend_fast_path_and_iana_path_agree_on_same_fixture_utc() {
        ...
        let mut evt1 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-trend-1", busytok_domain::AgentKind::ClaudeCode,
        );
        evt1.timestamp_ms = ts1;
        ...
        let mut evt2 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-trend-2", busytok_domain::AgentKind::Codex,
        );
        evt2.timestamp_ms = ts2;
        ...
        let iana = read_overview_trend_from_daily_usage(
            conn, rtz.canonical_name(), &date, &date, "gen-trend-contract",
        ).unwrap();
        ...
        assert_eq!(fast_tokens, iana_tokens,
        ...
        let mut evt3 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-trend-3", busytok_domain::AgentKind::ClaudeCode,
        );
        evt3.timestamp_ms = ts1 + 3600_000;
        evt3.total_tokens = 50;
        evt3.cost_usd = Some(0.005);
        let inserted_gen2 = insert_usage_events_batch_returning_inserted(
            conn, &[evt3], "gen-other",
    ...
    fn overview_heatmap_iana_path_reads_from_daily_usage_consistently() {
        ...
        let mut evt1 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-heatmap-1", busytok_domain::AgentKind::ClaudeCode,
        );
        evt1.timestamp_ms = ts1;
        evt1.total_tokens = 100;
        evt1.cost_usd = Some(0.01);
        ...
        let mut evt2 = busytok_domain::NormalizedUsageEvent::minimal_for_test(
            "evt-heatmap-2", busytok_domain::AgentKind::ClaudeCode,
        );
        evt2.timestamp_ms = ts2;
        ...
        let rows = read_overview_trend_from_daily_usage(
            conn, rtz.canonical_name(), &date1, &date1, "gen-heatmap",
        ).unwrap();
