...
- **Credential injection at spawn time (spec §2.3):** the service reads the API key from keychain (via the `credential_reader` seam — production wiring wraps `ProviderCredentialStore::get_key(provider_id)`; see Task 2 "Credential reader seam") and injects it into the child process env as `OPENAI_API_KEY=<key>` AND `provider.api_key_env_name=<key>` (both names, so the Pi SDK reads `OPENAI_API_KEY` by default while the provider-specific name is preserved for observability). The base URL is injected as `OPENAI_BASE_URL=<base_url>` AND `provider.base_url_env_name.unwrap_or("OPENAI_BASE_URL")=<base_url>`. The key NEVER persists to disk, NEVER enters logs, NEVER travels over network. It travels only via local IPC (Unix socket) from keychain to the service, then via env var from service to the sidecar child process.
...
- **Usage bridge to `usage_events` (spec §4 Phase 3 usage flow):** the **runtime handler** (in `busytok-runtime`'s `subagent_delegate` handler, NOT the `SidecarTaskExecutor` in `busytok-subagent` — see the next bullet for why) normalizes the sidecar's raw usage (`input_tokens`, `output_tokens`, `model`, `provider`) into a `NormalizedUsageEvent` with `client_kind: "subagent"`, computes `total_tokens = input + output` in Rust, looks up `cost_usd` via `busytok_pricing::estimate_cost_with_catalog` (a FREE FUNCTION in the `busytok_pricing` crate root, NOT a method on `PriceCatalog` — verified in `crates/busytok-pricing/src/lib.rs:534`), and writes the event to `usage_events` via `Database::ingest_store_batch` (the public API; `upsert_usage_events_dedup_aware` is an internal free function taking `&Connection`, not a `Database` method — verified in `crates/busytok-store/src/write_queries.rs:255`). The existing `subagent_usage_records` write (inside `SubagentManager`, internal bookkeeping) remains. The Activity page and Overview page pick up subagent usage naturally from `usage_events`.
...
**Interfaces:**
- Produces: `WorkerPool` struct owning `HashMap<String, Arc<PiSidecarSupervisor>>` + base `SidecarConfig` + `Arc<Mutex<Database>>` + provider config lookup closure + **credential reader closure** + shared `Arc<PressureGate>` + responder-factory closure
- Produces: `WorkerPool::new(base_config, db, providers, credential_reader, pressure_gate) -> Self` where `providers` is a provider config lookup (`Arc<dyn Fn(&str) -> Option<ProviderConfig> + Send + Sync>`) and `credential_reader` is `Arc<dyn Fn(&str) -> Result<Option<String>> + Send + Sync>` (the credential seam — see below), and `pressure_gate: Option<Arc<PressureGate>>` is the shared gate passed to every supervisor. The responder-factory is NOT passed to `new` — it's set via `set_responder_factory` after the executor is constructed (P1c fix: two-phase init).
- **Credential reader seam (P1 fix — testability):** `WorkerPool` does NOT call `ProviderCredentialStore::get_key` directly. Instead it owns a `credential_reader: Arc<dyn Fn(&str) -> Result<Option<String>> + Send + Sync>` closure, injected via `new`. This is necessary because `ProviderCredentialStore::get_key` is a static function with no injectable seam (verified at `crates/busytok-config/src/providers.rs:62` — `pub fn get_key(provider_id: &str) -> Result<Option<String>>` on a unit struct, no `self`); calling it directly from `ensure_worker` would force `WorkerPool` unit tests onto the real OS Keychain or heavy global monkey-patching, making `ensure_worker_fails_for_missing_api_key` / `ensure_worker_fails_for_keychain_error` / `ensure_worker_injects_credentials` impractical to write stably. The production wiring (in `construct_sidecar`, Task 4) is trivial: `Arc::new(|pid| ProviderCredentialStore::get_key(pid))`. Tests inject a canned closure (e.g. `Arc::new(|_| Ok(Some("test-key".to_string())))` or `Arc::new(|_| Err(anyhow!("simulated keychain failure")))`). This mirrors the existing `providers` closure pattern in the same struct for consistency.
...
- Produces: `WorkerPool::ensure_worker(&self, provider_id: &str) -> Result<Arc<PiSidecarSupervisor>, SidecarError>` — SYNCHRONOUS (not async — I2 fix: the body is entirely sync: credential read + config build + supervisor alloc + responder set + insert; no `.await`). **Locking:** (1) read credential via `self.credential_reader(provider_id)` OUTSIDE the map lock (credential read may be OS keychain I/O — I2 fix: `ProviderCredentialStore::get_key` can take 10-100ms+ on macOS, must not serialize all providers); (2) acquire map lock, check if entry exists (someone else may have created it while we read credentials), if yes → return existing + drop key; (3) if no entry → build config + construct supervisor + construct responder via factory (panics if `set_responder_factory` not yet called — P1c fail-fast) + `sup.set_pressure_responder(responder)` (C6 fix) + insert + return Arc.
...
Use a mock provider config map + a **mock `credential_reader` closure** (injected via `WorkerPool::new` — see the "Credential reader seam" interface above; do NOT touch the real OS Keychain in unit tests). For `ensure_worker_injects_credentials`, inject `Arc::new(|_| Ok(Some("test-key".to_string())))` and assert the constructed env map contains the key under both `OPENAI_API_KEY` and the provider's `api_key_env_name`. For `ensure_worker_fails_for_missing_api_key`, inject `Arc::new(|_| Ok(None))`. For `ensure_worker_fails_for_keychain_error`, inject `Arc::new(|_| Err(anyhow!("simulated keychain failure")))`. The tests should NOT spawn real sidecar processes — they verify the config/env construction, not the process lifecycle.
...
- `credential_reader: Arc<dyn Fn(&str) -> Result<Option<String>> + Send + Sync>` (P1 fix — credential seam; production wiring is `Arc::new(|pid| ProviderCredentialStore::get_key(pid))` in Task 4; tests inject canned closures)
...
2. Read API key: `(self.credential_reader)(provider_id)` (P1 fix — no lock held; the closure MAY be OS keychain I/O, 10-100ms+ on macOS; in tests it's an in-memory canned closure). Propagate `Err` as `SidecarError::Spawn("keychain read failed: ...")`. If `Ok(None)` → `SidecarError::Spawn("no API key for provider")`. Do NOT call `ProviderCredentialStore::get_key` directly here — that would bypass the seam and make unit tests depend on the real OS Keychain.
3. Acquire the workers map lock. Re-check if entry exists (someone else may have created it while we read credentials — avoids wasted supervisor construction). If yes → return existing Arc (discard the key we just read).
...
- Produces: `async fn WorkerPool::remove_worker_and_kill(provider_id)` — self-contained kill + remove (P1b fix: calls the existing `PiSidecarSupervisor::force_kill()` — no new kill method, M1 fix: reuse existing `force_kill`). The interface is defined in Task 2; Task 3 just calls it on auth failure.
...
4. Build the production credential reader closure (P1 fix): `let credential_reader: Arc<dyn Fn(&str) -> Result<Option<String>> + Send + Sync> = Arc::new(|pid| ProviderCredentialStore::get_key(pid));` — this is the thin wrapper that delegates to the static keychain API, giving `WorkerPool` an injectable seam for tests.
5. **Phase 1:** Construct `WorkerPool::new(base_config, Some(db), providers_lookup, credential_reader, Some(Arc::clone(&pressure_gate)))` — the responder-factory `OnceLock` is UNSET at this point.
6. **Phase 1:** Construct `SidecarTaskExecutor::with_pool(Arc::clone(&pool), db)` — captures `Arc<WorkerPool>`.
7. **Phase 2:** `pool.set_responder_factory(Arc::new({ let weak_exec = Arc::downgrade(&sidecar_executor); let gate = Arc::clone(&pressure_gate); move |weak_sup| Arc::new(PressureResponder::new(weak_sup, weak_exec.clone(), gate.clone())) }))` — injects the factory that captures `Weak<SidecarTaskExecutor>` + `Arc<PressureGate>`. This replaces the existing `assemble_with_sidecar` block at supervisor.rs:432-445 which constructed a single responder for the single supervisor.
8. Store `worker_pool: Some(Arc::clone(&pool))` on `BusytokSupervisor`.
9. Pass `Some(pressure_gate)` to `SubagentManager::with_pressure_gate(...)` (same gate — manager pauses the queue under pressure).
