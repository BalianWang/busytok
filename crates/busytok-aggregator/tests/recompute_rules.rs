...
fn date_rollups_respect_timezone_boundaries() {
    ...
    let date_plus8 = date_from_timestamp_ms(
        ts,
        &busytok_domain::ReportingTimezone::parse("+08:00").unwrap(),
    )
    .unwrap();
    assert_eq!(date_plus8, "2026-05-18");

    // 2026-05-18 03:00 UTC = 2026-05-17 20:00 -07:00 (previous day)
    let date_minus7 = date_from_timestamp_ms(
        ts,
        &busytok_domain::ReportingTimezone::parse("-07:00").unwrap(),
    )
    .unwrap();
    assert_eq!(date_minus7, "2026-05-17");

    // Same timestamp, UTC
    let date_utc = date_from_timestamp_ms(
        ts,
        &busytok_domain::ReportingTimezone::parse("UTC").unwrap(),
    )
    .unwrap();
