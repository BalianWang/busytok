...
impl ControlTransport for InMemoryTransport {
    ...
    async fn connect(endpoint: &str) -> Result<Self::Stream> {
        ...
        tx.send(server)
            .await
            .context("listener dropped before connect")?;
        Ok(client)
    }
}
...
mod tests {
    ...
    async fn roundtrip_through_duplex() {
        ...
        let accept_task =
            tokio::spawn(async move { InMemoryTransport::accept(&listener).await.unwrap() });
    ...
    async fn bind_and_connect_on_different_worker_threads() {
        ...
        let connect_task =
            tokio::spawn(async move { InMemoryTransport::connect(&ep_clone).await.unwrap() });
