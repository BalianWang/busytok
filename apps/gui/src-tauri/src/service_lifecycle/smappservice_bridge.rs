...
mod ffi {
    ...
    fn bundle_is_properly_signed() -> bool {
        ...
        let path: *mut objc::runtime::Object = unsafe { msg_send![bundle, bundlePath] };
        if path.is_null() {
            return false;
        }
    ...
    fn acquire_service(kind: SMAppServiceKind) -> Result<*mut objc::runtime::Object> {
        unsafe {
            ...
            if service.is_null() {
                Err(anyhow!("SMAppService factory returned nil for {:?}", kind))
            } else {
                Ok(service)
            }
        }
...
mod tests {
    ...
    fn ffi_nsstring_input_is_null_terminated_for_objc() {
        #[cfg(target_os = "macos")]
        {
            let c_string = ffi::cstring_for_nsstring_input_for_test("com.busytok.service.plist")
    ...
    fn ffi_nsstring_input_rejects_embedded_nul() {
        ...
        {
            let result =
                ffi::cstring_for_nsstring_input_for_test("com.busytok.service.plist\0garbage");
    ...
    fn executor_plumbing_returns_inner_result() {
        ...
        let result: Result<u32> = run_via_executor(exec_ref, || Ok(7u32));
        assert_eq!(result.unwrap(), 7);

        let failing: Result<()> = run_via_executor(exec_ref, || Err(anyhow!("boom")));
