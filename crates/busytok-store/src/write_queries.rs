...
mod tests {
    ...
    fn upsert_daily_usage_for_events_respects_timezone_date() {
        ...
        upsert_daily_usage_for_events(db.conn(), &[evt.clone()], &rtz_shanghai, "gen-test").unwrap();
        upsert_daily_usage_for_events(db.conn(), &[evt.clone()], &rtz_utc, "gen-test").unwrap();

        let sh_date: String = db.conn().query_row(
            "SELECT date FROM daily_usage WHERE timezone = 'Asia/Shanghai'",
            [], |row| row.get(0),
        ).unwrap();
        let utc_date: String = db.conn().query_row(
    ...
    fn daily_usage_failure_rolls_back_usage_events_same_transaction() {
        ...
        let event_count: i64 = db
            ...
            .unwrap();
        assert_eq!(event_count, 0, "rolled-back event must not be visible outside tx");

        let daily_count: i64 = db
            .conn()
            .query_row("SELECT COUNT(*) FROM daily_usage", [], |row| row.get(0))
            .unwrap();
        assert_eq!(daily_count, 0, "rolled-back daily_usage must not be visible outside tx");
    }
}
