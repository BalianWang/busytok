...
fn recover_missing_generation_metadata_returns_none_on_empty_db() {
    let db = Database::open_in_memory().expect("db");
    let result = generation_commands::recover_missing_generation_metadata(&db, "UTC").expect("recover");
...
fn recover_missing_generation_metadata_uses_iana_timezone_for_daily_usage() {
    ...
    assert_eq!(date, "2026-06-12", "date must use Asia/Shanghai boundary, not UTC");
    assert_eq!(tz, "Asia/Shanghai", "timezone must match the recovery timezone");
    assert_eq!(gen_id, r.generation_id, "generation_id must match recovered generation");
